...
def move_files():
    # Создать папку "Ознакомительная папка" в папке "PythonPractice\Block_D"
    if not os.path.exists("PythonPractice\\Block_D\\Ознакомительная папка"):
        os.mkdir("PythonPractice\\Block_D\\Ознакомительная папка")

    for folder in ['тема A', 'тема B']:
        folder_path = os.path.join("PythonPractice\\Block_D\\Ознакомительная папка", folder)
        if not os.path.exists(folder_path):
            os.mkdir(folder_path)
    ...
    for file in files:
        if re.search("task_A", file):
            # Переместить файл в папку "тема A"
            os.replace(os.path.join(path, file), "PythonPractice\\Block_D\\Ознакомительная папка\\тема A\\" + file)
        elif re.search("task_B", file):
            # Переместить файл в папку "тема B"
            os.replace(os.path.join(path, file), "PythonPractice\\Block_D\\Ознакомительная папка\\тема B\\" + file)
...
def execute_files():
    # Получить список подпапок в папке folder_path
    folder_path = "PythonPractice\\Block_D\\Ознакомительная папка"
    subfolders = [f.path for f in os.scandir(folder_path) if f.is_dir()]
